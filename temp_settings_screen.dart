...
class TempSettingsScreen extends StatefulWidget {
  final String deviceId;

  const TempSettingsScreen({super.key, required this.deviceId});

  @override
  State<TempSettingsScreen> createState() => _TempSettingsScreenState();
}

class _TempSettingsScreenState extends State<TempSettingsScreen> {
  ...
  void initState() {
    ...
    final device = Provider.of<DeviceProvider>(context, listen: false)
        .devices
        .firstWhere((d) => d.id == widget.deviceId, orElse: () => Device(id: "error", customName: "Error", deviceType: "TEMP"));
  ...
  void _onPwmMinChanged() {
    ...
    _pwmMinDebounce = Timer(const Duration(milliseconds: 1000), () {
      ...
      if (value != null && value >= 0 && value <= 255) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Invalid PWM Min. Range: 0-255'), backgroundColor: Colors.red),
        );
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    return Consumer<DeviceProvider>(
      builder: (context, deviceProvider, child) {        final device = deviceProvider.devices.firstWhere(
              (d) => d.id == widget.deviceId,
          orElse: () => Device(id: widget.deviceId, customName: 'Unknown TEMP Device', deviceType: 'TEMP'),
        );
        ...
        if (!_pwmMinFocusNode.hasFocus && device.pwmMin?.toString() != _pwmMinController.text) {
          _pwmMinController.text = device.pwmMin?.toString() ?? '';
            ...
            title: Text('${device.customName} (TEMP)'),
          ...
          body: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                const SizedBox(height: 10),                _buildInfoCard(device),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text('Device Type: ${device.deviceType.toUpperCase()}',
            ...
            _buildInfoRow('PWM Value:', '${device.pwmValue ?? "N/A"} (${device.pwmValue != null ? (device.pwmValue! / 255 * 100).toStringAsFixed(0) : "N/A"}%)'),
            ...
            _buildInfoRow('PWM Min (0-255):', '${device.pwmMin ?? "N/A"}'),
          ],
        ),
      ),
    );
  ...
  Widget _buildInfoRow(String label, String value) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Temperature Controls', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        const SizedBox(height: 10),
        TextField(
