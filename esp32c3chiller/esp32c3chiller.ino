...
Adafruit_SH1106 display(I2C_SDA_PIN, I2C_SCL_PIN); // SH1106 디스플레이
...
void save_all_settings() {
  ...
  if (EEPROM.commit()) {
    ...
    display_message("Settings Saved:",
                    ...
                    "Wiring: " + (wiring_topic.length() > 0 ? "Set" : "None"));
  } else {
...
void process_and_update() {
  ...
  if (temperature_received) {
    ...
    sprintf(buffer, "Temp: %.2f C", received_temperature);
    display.println(buffer);
    sprintf(buffer, "Set : %.1f C", current_set_temp);
    display.println(buffer);
    sprintf(buffer, "Cool: %s", chiller_state ? "ON" : "OFF");
    display.println(buffer);
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (wiring_topic.length() > 0 && String(topic) == wiring_topic) {
    ...
    if (!tempError && tempDoc.containsKey("currentTemp")) {
      ...
      if (temp > -50.0 && temp < 100.0) {
        ...
        Serial.printf("Wiring에서 온도 수신 (JSON): %.2f C\n", received_temperature);
      }
    } else {
      // 단순 숫자 형태
